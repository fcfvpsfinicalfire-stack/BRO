...
const PlanCard: React.FC<{ plan: typeof budgetPlans[0] }> = ({ plan }) => {
    ...
    return (
        <div className={`relative bg-card-bg-solid border rounded-2xl p-8 flex flex-col h-full transition-all duration-300 group ${plan.isPopular ? 'border-2 border-discord-blue shadow-2xl shadow-discord-blue/20 bg-gradient-to-b from-[#111222] to-discord-blue/10' : 'border-white/10 hover:border-discord-blue/50 hover:-translate-y-1'}`}>
            ...
            <button className={`w-full mt-auto py-3 px-6 font-bold rounded-lg transition-all duration-300 ${plan.isPopular ? 'bg-discord-blue text-white hover:opacity-90' : 'bg-white/10 text-white hover:bg-white/20'}`}>
              Get Started
            </button>
        </div>
    );
...
const Background = () => (
    <div className="absolute inset-0 z-0 overflow-hidden bg-dark-bg">
        <div className="absolute inset-0 animate-aurora opacity-50" style={{
            background: 'linear-gradient(125deg, rgba(58, 89, 224, 0.15) 0%, rgba(13, 14, 27, 0) 25%, rgba(167, 85, 247, 0.1) 50%, rgba(13, 14, 27, 0) 75%, rgba(58, 89, 224, 0.15) 100%)',
            backgroundSize: '400% 400%',
        }}></div>
        <div 
            className="absolute inset-[-50%] animate-drift"
            style={{
                backgroundImage: 'radial-gradient(circle, rgba(58, 89, 224, 0.12) 1px, transparent 1px)',
        ...
        <div 
            className="absolute inset-[-50%] animate-drift"
            style={{
                backgroundImage: 'radial-gradient(circle, rgba(167, 85, 247, 0.1) 1px, transparent 1px)',
                backgroundSize: '3.5rem 3.5rem',
                animationDirection: 'reverse',
                animationDuration: '80s',
            }}
        ></div>
        <div className="absolute inset-0 bg-gradient-to-b from-dark-bg via-dark-bg/50 to-dark-bg"></div>
    </div>
);
                ...
                <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center md:text-left">
                     ...
                     <div className="relative h-64 md:h-full flex items-center justify-center min-h-[300px]">
                        <div className="relative w-64 h-64 animate-float-alt">
                            {/* Main Discord Icon */}
                            <div className="absolute inset-0 flex items-center justify-center">
                                <i className="fa-brands fa-discord text-[200px] text-discord-blue opacity-80" style={{ textShadow: '0 0 50px rgba(58, 89, 224, 0.7)' }}></i>
                            </div>
                            {/* Floating code icons */}
                            <div className="absolute -top-4 -left-4 p-4 bg-card-bg-solid rounded-xl border border-white/10 shadow-lg animate-subtle-bob" style={{ animationDelay: '0.5s' }}>
                                <i className="fa-brands fa-node-js text-4xl text-green-300"></i>
                            </div>
                            <div className="absolute top-8 right-0 p-4 bg-card-bg-solid rounded-xl border border-white/10 shadow-lg animate-subtle-bob" style={{ animationDelay: '1s' }}>
                                <i className="fa-solid fa-database text-4xl text-purple-300"></i>
                            </div>
                            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 p-4 bg-card-bg-solid rounded-xl border border-white/10 shadow-lg animate-subtle-bob" style={{ animationDelay: '1.5s' }}>
                                <i className="fa-solid fa-code text-4xl text-indigo-300"></i>
                            </div>
                        </div>
                    </div>
                </div>
